...
struct FilterCondition {
  ...
  /// 但那是**表达方式**的选择，不是语义的一部分：平台上若有现成的「不等于」匹配，
  /// 一条就够，拆成正向区间反而又多又宽。所以这里原样带着标记往下传，
  /// 由平台层挑最省的表达（见 `filter_plan_wfp.cpp`）。
  ///
  /// ⚠️ 平台层必须注意：**同一字段上如果不止一个条件，就不能一律翻成「不等于」**。
  /// 多个条件是「或」，而 `≠A 或 ≠B` 恒真；这种情形**只能先把集合求补**，
  /// 再用求补得到的正向取值各出一个条件。只有单值取反才能直接用「不等于」。
  bool negate = false;
